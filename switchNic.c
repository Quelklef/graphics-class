...
double polygonXs[10][10000][10000];
double polygonYs[10][10000][10000];
double flex[10000][10000];
int pointCount[10];
...
void collectPoints(char *filename, int index) {
  ...
  fscanf(file, "%d", &pointCount[i]);

  // Initialize (x, y) arrays to store points
  double xs[pointCount[index]];
  double ys[pointCount[index]];
...
int main(int argc, char **argv) {
  ...
  for(int i = 1; i < argc; i++)  {
    ...
    if (keyCode == prevKeyCode) {
      ...
      for(int i = 0; i < polygonCount; i++){
	printf("here\n");
	      M2d_mat_mult_points(polygonXs[i], polygonYs[i], Rot, polygonXs[i], polygonYs[i], polygonPointCounts[i]);
} 
       printf("plz\n");
       colorPoly();
       M2d_print_mat(Rot);
